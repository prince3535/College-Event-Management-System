...
Label(root, text = 'COMPUTER DEPARTMENT', font =('Calibri', 13)).pack(side = TOP, pady = 10)
def action_add(a):
...
def adding(event,e1):
	...
	sql = "CREATE TABLE `project`.`feedback_comp_"+a+"` (`Name` VARCHAR(20) NOT NULL ,`Feedback` varchar(50) NOT NULL ,`Location` VARCHAR(50) NOT NULL,`Phone_no.` varchar(10) NOT NULL)"
	print(sql)
	cursor.execute(sql)
...
def feedback_sql(fed):
	...
	if (fed=='semi' or fed=='spor' or fed=='tech'):
		v1=tk.IntVar()
		...
		tk.Radiobutton(feed, 
			  text="ENETR FEEDBACK",
			  padx = 20, 
			  variable=v1, activebackground="red",activeforeground="white",value=1,command=lambda *args:FeedbackEntryPoint(feed,'comp')).place(x=0,y=0)
		tk.Radiobutton(feed, 
			  text="VIEW FEEDBACK",
			  padx = 20, 
			  variable=v1,activebackground="red",activeforeground="white", 
			  value=2,command=lambda *args:feedbackShow_action(feed,'comp')).place(x=0,y=50)
...
def feedbackShowSelect(feed):
    ...
    Label(mainframe, text="PLEASE HAVE YOUR CHOICE:").grid(row = 1, column = 1)
    popupMenu.grid(row = 2, column =1)
    tk.Button(feed,text='show',command=lambda *args: feedbackShow_action(feed,tkvar)).place(x=500,y=180)
...
def FeedbackEntryPoint(feed,tkvar):
	if tkvar == 'comp':
		...
		root.title("Feedback")
		h='Feedback For Computer Department'
		...
		v2 = IntVar()
		t="What remark do you like to give for Computer Department ?"
		...
		t="Please mention the name of "+tkvar.get()+" Event for which you want to give feedback :"
		tk.Label(new,text=t).place(x=0,y=160)
...
def feedbackEntry_action(new,feed,tkvar,e1,e2,e3,e4,e5):
	...
	if (e1.get()=='' or e2.get()=='' or e3.get()=='' or e4.get()=='' or org=='nun' or co=='nun'):
		...
		connection = pymysql.connect(host="localhost",user="root",passwd="",database="project")
		if tkvar=='comp':
			print(e1.get(),e5,e2.get(),e3.get(),org,co,e4.get())
			try:
				if e5.get()=='.!entry5':
					e5='-'
				else:
			...
			try:
				if e5.get()=='':
					...
					e5=e5.get()
				cursor = connection.cursor()
				sql = "INSERT INTO `feedback_comp_"+tkvar.get()+"` (`Name`, `ExtraFeedback`, `Location`, `Phone_no.`, `ManagementOfEvent`, `CoordinatorReview`, `EventName`) VALUES (%s, %s, %s, %s,%s,%s,%s)"
				# Execute the query
				cursor.execute(sql, (e1.get(),e5,e2.get(),e3.get(),org,co,e4.get()))
...
def feedbackShow_action(feed,tkvar):
	...
	if tkvar=='comp':
		mycursor.execute("SELECT * FROM feedback_comp")
		obj=mycursor.fetchall()

		row_count=len(obj)
		i=0
		Label(frame, text = 'FEEDBACK VIEW WINDOW of Computer Department\n'+tkvar.get(), font =('Candara Bold Italic', 16)).pack(side = TOP, pady = 10)
		while i<row_count:
			pr=('NAME:',obj[i][0])
...
def img_view(v):
    if v=='bu':
        webbrowser.open_new(("https://drive.google.com/open?id=1qpappU5hH8F6Act2fvV__uGBZznZJ3Bs"))
...
def action(n):
    if n==1:
        ...
        Label(root,text='BUCKLE UP',font=('Verdana',15)).pack(pady=10)
        Label(root,text=' The event of BUCKLE UP was oranized in Smt. Indira Gandhi College Of Engineering by Department of Computers on \n             26 September 2019.\nThe event occured on large scale students from different college had taken part into this event.\nThe team of organizers was so focused that the event went so smoothly.\nK.T Patil sir(DEAN) and other Teacher staff supported a lot.',font=('Verdana',10)).pack(pady=10)
...
file = Menu(menu)
menu.add_cascade(label="FEEDBACK", menu=file)
file.add_command(label="Feedback for Computer Department",activebackground="green",activeforeground="yellow",command=lambda *args:feedback_sql('comp'))
