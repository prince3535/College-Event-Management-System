...
root.title("Electronic DEPARTMENT")
Label(root, text = 'Smt. Indira Gandhi College Of Engineering Events', font =('Verdana', 15)).pack(side = TOP, pady = 10)
Label(root, text = 'Electronic DEPARTMENT', font =('Calibri', 13)).pack(side = TOP, pady = 10)
def action_add(a):
...
def adding(event,e1):
	...
	sql = "CREATE TABLE `project`.`feedback_electronic_"+a+"` (`Name` VARCHAR(20) NOT NULL ,`Feedback` varchar(50) NOT NULL ,`Location` VARCHAR(50) NOT NULL,`Phone_no.` varchar(10) NOT NULL)"
	print(sql)
	cursor.execute(sql)
	...
	f1.close()
	fele=open("ElectronicEventAdd.py","a")
	fele.write("\n")
	tran=''
	tran="a='"+a+"'"
	fele.write(tran)
...
def feedback_sql(fed):
    ...
    if (fed=='semi' or fed=='spor' or fed=='tech'):
        ...
        tk.Radiobutton(feed, 
              text="ENETR FEEDBACK",
              padx = 20, 
              variable=v1, activebackground="red",activeforeground="white",value=1,command=lambda *args:FeedbackEntryPoint(feed,'electronic')).place(x=0,y=0)
        tk.Radiobutton(feed, 
              text="VIEW FEEDBACK",
              padx = 20, 
              variable=v1,activebackground="red",activeforeground="white", 
              value=2,command=lambda *args:feedbackShow_action(feed,'electronic')).place(x=0,y=50)
...
def FeedbackEntryPoint(feed,tkvar):
	if tkvar == 'electronic':
		...
		root.title("Feedback")
		h='Feedback For Electronic Department'
		...
		if tkvar=='electronic':
			...
			try:
				if e5.get()=='.!entry5':
					...
					e5=e5.get()
				cursor = connection.cursor()
				sql = "INSERT INTO `feedback_electronic_"+tkvar.get()+"` (`Name`, `ExtraFeedback`, `Location`, `Phone_no.`, `ManagementOfEvent`, `CoordinatorReview`, `EventName`) VALUES (%s, %s, %s, %s,%s,%s,%s)"
				# Execute the query
				cursor.execute(sql, (e1.get(),e5,e2.get(),e3.get(),org,co,e4.get()))
...
def feedbackShow_action(feed,tkvar):
    ...
    if tkvar=='electronic':
        mycursor.execute("SELECT * FROM feedback_electronic")
        obj=mycursor.fetchall()

        row_count=len(obj)
        i=0
        Label(frame, text = 'FEEDBACK VIEW WINDOW of Electronic Department', font =('Candara Bold Italic', 16)).pack(side = TOP, pady = 10)
        while i<row_count:
            pr=('NAME:',obj[i][0])
        ...
        quere="SELECT * FROM feedback_electronic_"+tkvar.get()
...
file = Menu(menu)
menu.add_cascade(label="FEEDBACK", menu=file)
file.add_command(label="Feedback for Electronic Department",activebackground="green",activeforeground="yellow",command=lambda *args:feedback_sql('electronic'))
