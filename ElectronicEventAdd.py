...
def feedbackEntry_action(feed,tkvar,e1,e2,e3,e4):
	...
	if (e1.get()=='' or e2.get()=='' or e3.get()=='' or e4.get()=='' or tkvar.get()==''):
		...
		try:
			...
			sql = "INSERT INTO `feedback_electronic_"+tkvar.get()+"` (`Name`,`Feedback`,`Location`,`Phone_no.`) VALUES (%s, %s, %s, %s)"
			# Execute the query
...
file.add_command(label="Feedback for NEW EVENTS",activebackground="green",activeforeground="yellow",command=lambda *args:feedback_sql())
n=0